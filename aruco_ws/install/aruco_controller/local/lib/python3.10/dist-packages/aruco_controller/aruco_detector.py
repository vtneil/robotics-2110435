...
class ArucoDetector(Node):
    ...
    def __init__(self):
        ...
        self.marker_id = self.marker_ids #[int(os.getenv('ROS_DOMAIN_ID'))]
        ...
        self.marker_array_publisher = self.create_publisher(MarkerArray, "~/marker_array", 1)
    ...
    def detect_marker(self) -> Tuple[Bool, Optional[List[Tuple]]]:
        """
        ...
        if detection_found:
            ...
            for id in self.marker_id:
                ...
                if found_result:
                    ...
                    # Return Result
                    marker = (rvec, tvec, id)
                    marker_lists.append(marker)
            
            if len(marker_lists) > 0:
                return True, marker_lists #rvec, tvec, "fiducial"
    ...
    def process(self):
        ...
        marker_found, marker_lists = self.detect_marker()
        if marker_found:
            # Put Rvec Tvec into the KALMAN FILTER
            # self.update_kalman(rvec, tvec)
            # Publish Detection Result
            self.pub_detections(marker_lists)#rvec, tvec, frame_name)
        else:
    ...
    def pub_detections(self, marker_lists): #rvec, tvec, frame_name 
        """
        Publish Detections
        """
        marker_list_msg = []
        for marker in marker_lists:
            ...
            marker_msg = self.marker_msg_from_marker_tf(marker_tf)
            marker_list_msg.append(marker_msg)
        ...
        marker_array.header = marker_tf.header
        marker_array.markers = marker_list_msg
        
        self.marker_array_publisher.publish(marker_array)
    
    def marker_msg_from_marker_tf(self, marker_tf:TransformStamped):
        pose_cov_stamped = pose_with_cov_from_transform_stamped(marker_tf)
        pose_cov_stamped.header.stamp = marker_tf.header.stamp # timeStamp
        # Covariance of Detection
        pose_cov_stamped.pose.covariance[6*0+0] = 0.1 * 0.1
        pose_cov_stamped.pose.covariance[6*1+1] = 0.1 * 0.1
        pose_cov_stamped.pose.covariance[6*5+5] = math.pi/24.0 * math.pi/24.0
        
        marker_msg = Marker()
        marker_msg.header = pose_cov_stamped.header
        marker_msg.pose = pose_cov_stamped.pose
        marker_msg.id = int(marker_tf.child_frame_id.split("_")[-1])
        marker_msg.confidence = 100.0
